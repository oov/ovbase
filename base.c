...
static void global_hint_init(void)
{
  ...
  if (QueryPerformanceFrequency(&f) != 0 && QueryPerformanceCounter(&c) != 0) {
    ...
    atomic_store_explicit(&g_global_hint, GetTickCount() + GetCurrentProcessId() + GetCurrentThreadId(), memory_order_relaxed);
  }
#else
  struct timespec v = {0};
...
#ifdef _WIN32
  wsprintfW(buf, NEWLINE NSTR("(error code: %02X:0x%08X)") NEWLINE NSTR("  %hs:%ld %hs()"), e->type, e->code, e->filepos.file, e->filepos.line, e->filepos.func);
#else
  sprintf(buf, NEWLINE NSTR("(error code: %02X:0x%08X)") NEWLINE NSTR("  %s:%ld %s()"), e->type, e->code, e->filepos.file, e->filepos.line, e->filepos.func);
#endif
  err = scat(&tmp, buf);
  if (efailed(err))
  {
